...
{    
    ...
    */
    public $changelog = "";
    public $database = "";
    /** @var \SimpleXMLElement  $currentBd*/
    ...
    public function run($args) {        
        ...
        preg_match('/dbname=([0-9,a-z,A-Z$_]+);?/',Yii::$app->db->dsn,$matches);
        $this->database = $matches[1];
        $tables = Yii::$app->db->schema->getTableSchemas($this->database);
    ...
    private function createMigration($name) {
        ...
        {
            $output = Array();
            exec(Yii::$app->getBasePath() . DIRECTORY_SEPARATOR  . 'yii migrate/create '.$name.' --interactive=0', $output);
            echo implode("\n", $output);
        }
        catch(\Exception $ex)
        {
    ...
    private function markMigration($name) {
        ...
        {
            $output = Array();
            exec(Yii::$app->getBasePath() . DIRECTORY_SEPARATOR  . 'yii migrate/mark '.$name.' --interactive=0', $output);
            echo implode("\n", $output);
        }
        catch(\Exception $ex)
        {
